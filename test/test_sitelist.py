...
class TestSiteListDateHandling(unittest.TestCase):
    def test_dates(self):
        #normal good date
        self.assertEqual(helper.get_datetime('2012-01-13T12:30:00+00:00'),
                         datetime(2012, 1, 13, 12, 30, 0))
        #bad date (month 13)
        self.assertRaises(ValueError, helper.get_datetime, '2012-13-13T12:30:00+00:00')

    def test_goodtimezones(self):        
        date = '2012-01-01T12:20:00'
        dt = datetime(2012, 1, 1, 12, 20, 0)
        #utc
        self.assertEqual(helper.get_datetime(date + '-00:00'), dt)   
        self.assertEqual(helper.get_datetime(date + '+00:00'), dt)   

        #positive timezone 
        self.assertEqual(helper.get_datetime(date + '+01:30'),
    ...
    def test_badtimezones(self):
        date = '2012-01-01T12:20:00'
        #invalid timezones
        self.assertRaises(ValueError, helper.get_datetime, date + 'aaaaaa')
        self.assertRaises(ValueError, helper.get_datetime, date + '+aa:aa')
        self.assertRaises(ValueError, helper.get_datetime, date + '+15:a0')

class TestSiteListUnusualTimes(unittest.TestCase):
    def test_noMillis_noTimezone(self):
        # missing "+00:00" or similar
        self.assertEqual(helper.get_datetime('2012-01-01T12:30:00'),
                         datetime(2012, 1, 1, 12, 30, 0))

    def test_millis_timezones(self):
        # with millis appended - just flooring that value for the moment
        self.assertEqual(helper.get_datetime('2012-01-01T12:30:00.123+01:00'),
                         datetime(2012, 1, 1, 11, 30, 1))

    def test_millis_noTimezone(self):
        # with millis and no tz
        self.assertEqual(helper.get_datetime('2012-01-01T12:30:00.123'),
                         datetime(2012, 1, 1, 12, 30, 1))


    def test_noColonTimezone(self):
        #timezone as +0000
        self.assertEqual(helper.get_datetime('2012-01-01T12:30:00+0130'),
    ...
    def test_noMinutesTimezone(self):
        ...
        self.assertEqual(helper.get_datetime('2012-01-01T12:30:00+01'),
...
class TestSiteListFetchHandling(unittest.TestCase):
    ...
    datetimeA = helper.get_datetime('2011-04-07T12:10:01-00:00')
    datetimeB = helper.get_datetime('2012-05-08T12:11:02-01:20')
    datetimeC = helper.get_datetime('2013-06-09T12:12:03+03:40')
